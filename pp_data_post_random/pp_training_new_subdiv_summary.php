...
if($posted_date != 'ALL'){
    $subdiv_query=$mysqli->prepare("SELECT subdivision.subdivisioncd, subdivision.subdivision, COUNT(personnel_new.personcd) FROM subdivision INNER JOIN personnel_new ON personnel_new.subdivisioncd=subdivision.subdivisioncd WHERE DATE(personnel_new.posted_date) = ? GROUP BY subdivision.subdivisioncd, subdivision.subdivision ORDER BY subdivision.subdivisioncd") or die($mysqli->error);
    $subdiv_query->bind_param("s",$posted_date) or die($subdiv_query->error);
}
else{
    $subdiv_query=$mysqli->prepare("SELECT subdivision.subdivisioncd, subdivision.subdivision, COUNT(personnel_new.personcd) FROM subdivision INNER JOIN personnel_new ON personnel_new.subdivisioncd=subdivision.subdivisioncd GROUP BY subdivision.subdivisioncd, subdivision.subdivision ORDER BY subdivision.subdivisioncd") or die($mysqli->error);
}
...
if($posted_date != 'ALL'){
    $poststat_query=$mysqli->prepare("SELECT poststat.post_stat, poststat.poststatus, COUNT(personnel_new.personcd) FROM poststat INNER JOIN personnel_new ON poststat.post_stat=personnel_new.poststat WHERE DATE(personnel_new.posted_date) = ? GROUP BY poststat.post_stat, poststat.poststatus ORDER BY poststat.post_stat, poststat.poststatus") or die($mysqli->error);
    $poststat_query->bind_param("s",$posted_date) or die($poststat_query->error);
}
else{
    $poststat_query=$mysqli->prepare("SELECT poststat.post_stat, poststat.poststatus, COUNT(personnel_new.personcd) FROM poststat INNER JOIN personnel_new ON poststat.post_stat=personnel_new.poststat GROUP BY poststat.post_stat, poststat.poststatus ORDER BY poststat.post_stat, poststat.poststatus") or die($mysqli->error);
}
...
<table id="subdiv_booked_summary" class="table table-bordered table-condensed table-hover small">
    ...
    <tbody>
        <?php
        if($posted_date != 'ALL'){
            $subdiv_booked_query=$mysqli->prepare("SELECT subdivision.subdivisioncd, personnel_new.poststat, COUNT(personnel_new.personcd) FROM subdivision INNER JOIN personnel_new ON subdivision.subdivisioncd=personnel_new.subdivisioncd WHERE DATE(personnel_new.posted_date) = ? GROUP BY subdivision.subdivisioncd, personnel_new.poststat ORDER BY subdivision.subdivisioncd, personnel_new.poststat") or die($mysqli->error);
            $subdiv_booked_query->bind_param("s",$posted_date) or die($subdiv_booked_query->error);
        }
        else{
            $subdiv_booked_query=$mysqli->prepare("SELECT subdivision.subdivisioncd, personnel_new.poststat, COUNT(personnel_new.personcd) FROM subdivision INNER JOIN personnel_new ON subdivision.subdivisioncd=personnel_new.subdivisioncd GROUP BY subdivision.subdivisioncd, personnel_new.poststat ORDER BY subdivision.subdivisioncd, personnel_new.poststat") or die($mysqli->error);
        }
	...
	<tr>
        ...
        ?>
            ...
            <td class="text-center"><a href="pp_data_post_random/first_appt_letter_subdiv_new.php?subdiv_code=<?php echo $subdiv[$i]['SubdivCode']; ?>&posted_date=<?php echo $posted_date; ?>" class="btn btn-default btn-sm text-red" target="_blank"><i class="fa fa-print"></i></a></td> 
        </tr>
        <?php
        }
        ?>
