...
<html lang="en">
<head>
  <title>General Loksabha Election 2019 - Office Letter 11/PPCELL(Dist) 24.12.2018</title>
</head>
<body>
<?php
$opt = $_GET['opt'];
$code = $_GET['code'];
	$query_get_office=mysqli_query($DBLink,"SELECT offices.id, offices.name AS office_name, offices.officer_designation, offices.address,  block_munis.name AS block_muni_name, police_stations.name AS police_station_name, offices.post_office, offices.pin, offices.email, offices.phone, offices.mobile FROM ((offices INNER JOIN block_munis ON block_munis.id=offices.block_muni_id) INNER JOIN police_stations ON police_stations.id=offices.police_station_id) INNER JOIN training_no_office_nopp ON offices.id = training_no_office_nopp.office_id WHERE offices.block_muni_id='$code' ORDER BY offices.id") or die(mysqli_error($DBLink));
...
?>
    ...
    <tr>
		...
		<tr>
      <th width="50%" align="left">Memo No: 11/PPCELL(Dist)</th>
    ...
    <tr>
      ...
      <td colspan="2" align="justify">
        <p>&nbsp;&nbsp;&nbsp;&nbsp;WHEREAS, it has been found that inspite of service of letter regarding online submission of PP1 & PP2 data  in connection with General Parliament Election 2019 under this office memo no. 06(3)/PP Cell (dist)/Elec. Dated 29.11.2018 & 07/)/PP Cell (dist)/Elec. Dated 13.12.2018 you have neither attended training programme nor have initiated entry in PPMS portal.</p>
      ...
      <td width="50%" align="left">
          Received the copy with Memo No. 11/PPCELL(Dist)  dated. 24/12/2018<br><br>
