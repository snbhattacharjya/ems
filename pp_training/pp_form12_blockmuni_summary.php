...
$poststat_query=$mysqli->prepare("SELECT poststat.post_stat, poststat.poststatus, COUNT(personnel.personcd) FROM (poststat INNER JOIN personnel ON poststat.post_stat=personnel.poststat) INNER JOIN assembly ON assembly.assemblycd = personnel.acno WHERE personnel.poststat IN ('PR','P1','P2','P3') AND personnel.booked IN ('P','R') AND personnel.subdivisioncd = ? AND assembly.assemblycd != '999' GROUP BY poststat.post_stat, poststat.poststatus ORDER BY poststat.post_stat, poststat.poststatus") or die($mysqli->error);
...
<div class="text-center margin-bottom">
    <a class="btn btn-default btn-md" href="pp_training/pp_form12_blockmuni_summary_print.php?subdiv=<?php echo $subdiv_param; ?>" target="_blank">
...
<table id="blockmuni_exempt_summary" class="table table-bordered table-condensed table-hover small">
	...
	<tr>
        ...
        ?>
            ...
            <td class="text-center"><a href="pp_training/form12_print_blockmuni.php?block_muni_code=<?php echo $blockmuni[$i]['BlockMuniCode']; ?>" class="text-red" target="_blank"><i class="fa fa-print"></i> Print</a></td>
        </tr>
        <?php
        }
        ?>
...
<script>
    $('.office-pp-report-btn').click(function(e){
        ...
        loadOfficeForm12Summary(subdiv, blockmuni);
    });
