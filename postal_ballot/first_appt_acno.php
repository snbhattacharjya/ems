<title>
    AC No wise - 1st Appointment Letter
</title>
<?php
session_start();
...
$first_app_query=$mysqli->prepare("SELECT personcd, officer_name, off_desg, poststatus, mob_no, epic, partno, slno, acno, bank, branch, ifsc, bank_accno, officecd, office, address, block_muni_name, postoffice, subdivision, policestation, district, pin, training_desc, venuename, venueaddress, training_dt, training_time FROM first_rand_table WHERE acno = ? AND personcd NOT IN (SELECT personcd FROM personnel_training_absent) ORDER BY personcd") or die($mysqli->error);
$first_app_query->bind_param("s",$acno) or die($first_app_query->error);
