...
$blockmuni_query=$mysqli->prepare("SELECT block_muni.blockminicd, block_muni.blockmuni, COUNT(personnel_new.personcd) FROM (office INNER JOIN personnel_new ON office.officecd=personnel_new.officecd) INNER JOIN block_muni ON office.blockormuni_cd=block_muni.blockminicd WHERE office.subdivisioncd = ? AND personnel_new.poststat IN ('PR','P1','P2','P3') AND personnel_new.booked IN ('P','R') GROUP BY block_muni.blockminicd, block_muni.blockmuni ORDER BY block_muni.blockminicd, block_muni.blockmuni") or die($mysqli->error);
...
$poststat_query=$mysqli->prepare("SELECT poststat.post_stat, poststat.poststatus, COUNT(personnel_new.personcd) FROM poststat INNER JOIN personnel_new ON poststat.post_stat=personnel_new.poststat WHERE personnel_new.poststat IN ('PR','P1','P2','P3') AND personnel_new.booked IN ('P','R') AND personnel_new.subdivisioncd = ? GROUP BY poststat.post_stat, poststat.poststatus ORDER BY poststat.post_stat, poststat.poststatus") or die($mysqli->error);
...
<div class="text-center margin-bottom">
    <a class="btn btn-default btn-md" href="pp_data_post_random/pp_training_new_blockmuni_summary_print.php?subdiv=<?php echo $subdiv_param; ?>" target="_blank">
...
<table id="blockmuni_exempt_summary" class="table table-bordered table-condensed table-hover small">
    ...
    <tbody>
        <?php
        $blockmuni_booked_query=$mysqli->prepare("SELECT block_muni.blockminicd, personnel_new.poststat, COUNT(personnel_new.personcd) FROM (office INNER JOIN personnel_new ON office.officecd=personnel_new.officecd) INNER JOIN block_muni ON office.blockormuni_cd=block_muni.blockminicd WHERE office.subdivisioncd = ? AND personnel_new.poststat IN ('PR','P1','P2','P3') AND personnel_new.booked IN ('P','R') GROUP BY block_muni.blockminicd, personnel_new.poststat ORDER BY block_muni.blockminicd, personnel_new.poststat") or die($mysqli->error);
	...
	<tr>
        ...
        ?>
            ...
            <td class="text-center"><a href="pp_data_post_random/first_appt_letter_blockmuni_new.php?block_muni_code=<?php echo $blockmuni[$i]['BlockMuniCode']; ?>" class="btn btn-default btn-sm text-red" target="_blank"><i class="fa fa-print"></i></a></td>
        </tr>
        <?php
        }
        ?>
...
<script>
    $('.office-pp-report-btn').click(function(e){
        ...
        loadOfficePPNewSummary(subdiv, blockmuni);
    });
    
    $('.subdiv-summary').click(function(e){
        e.preventDefault();
        loadSubdivPPNewBookedSummary();
    });
    
    function loadSubdivPPNewBookedSummary(){
        $('.ajax-result').empty();
        $('.ajax-loader').show();
        //var data_target=$(this).attr('data-target').valueOf().toString();
        $.ajax({
                mimeType: 'text/html; charset=utf-8', // ! Need set mimeType only when run from local file
                url: "pp_data_post_random/pp_training_new_subdiv_summary.php",
                //type: "POST",
                //data: {
    ...
    function loadOfficePPNewSummary(subdiv, blockmuni){
        $('.ajax-result').empty();
        $('.ajax-loader').show();
        $.ajax({
                mimeType: 'text/html; charset=utf-8', // ! Need set mimeType only when run from local file
