...
$second_app_query->bind_result($personcd, $officer_name, $off_desg, $poststatus, $poststat, $mob_no, $epic, $partno, $slno, $acno, $bank, $branch, $ifsc, $bank_accno, $officecd, $office, $address, $block_muni_name, $postoffice, $subdivision, $policestation, $district, $pin, $training_desc, $venuename, $venueaddress, $training_dt, $training_time, $forassembly, $forassembly_name, $booked, $groupid, $dc_venue, $dc_addr, $rc_venue, $rc_addr, $memo, $date, $training_desc, $venuename, $venueaddress, $training_dt, $training_time) or die($second_app_query->error);
...
while($second_app_query->fetch()){
    if($booked == 'P'){
      ...
      $pp_data[]= array("personcd"=>$personcd, "officer_name"=>$officer_name, "off_desg"=>$off_desg, "poststatus"=>$poststatus, "poststat"=>$poststat, "mob_no"=>$mob_no, "epic"=>$epic, "partno"=>$partno, "slno"=>$slno, "acno"=>$acno, "bank"=>$bank, "branch"=>$branch, "ifsc"=>$ifsc, "bank_accno"=>$bank_accno, "officecd"=>$officecd, "office"=>$office, "address"=>$address, "block_muni_name"=>$block_muni_name, "postoffice"=>$postoffice, "subdivision"=>$subdivision, "policestation"=>$policestation, "district"=>$district, "pin"=>$pin, "training_desc"=>$training_desc, "venuename"=>$venuename, "venueaddress"=>$venueaddress, "training_dt"=>$training_dt, "training_time"=>$training_time, "forassembly"=>$forassembly, "forassembly_name"=>$forassembly_name, "booked"=>$booked, "groupid"=>$groupid, "dc_venue"=>$dc_venue, "dc_addr"=>$dc_addr, "rc_venue"=>$rc_venue, "rc_addr"=>$rc_addr, "memo"=>$memo, "date"=>$date, "training_dec"=>$training_dec, "venuename"=>$venuename, "venueaddress"=>$venueaddress, "training_dt"=>$training_dt, "training_time"=>$training_time);
    }
}
...
<table width="100%" style="font-family: sans-serif; font-size: 11">
    ...
    <tr>
        <td colspan="3" style="padding-top: 5; text-align: justify">
            You are requested to attend 2nd Training at <strong><?php echo $pp_data[$i]['venuename']; ?> on <strong><?php echo $pp_data[$i]['training_dt'];?></strong> from <strong><?php echo $pp_data[$i]['training_time'];?></strong>
        </td>
    </tr>
