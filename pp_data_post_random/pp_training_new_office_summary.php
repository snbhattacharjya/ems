...
$blockmuni_office_query=$mysqli->prepare("SELECT office.officecd, office.office, office.address1, COUNT(personnel_new.personcd) FROM office INNER JOIN personnel_new ON office.officecd=personnel_new.officecd GROUP BY office.officecd, office.office, office.address1 ORDER BY office.officecd") or die($mysqli->error);
...
<h3 class="page-header">
    New Personnel Office Summary
</h3>
<div class="text-center">
    <a class="btn btn-default btn-md" href="pp_data_post_random/pp_training_new_office_summary_print.php" target="_blank">
...
<table id="blockmuni_office_summary" class="table table-bordered table-condensed table-hover small">
    ...
    <tbody>
        <?php
        $blockmuni_office_booked_query=$mysqli->prepare("SELECT office.officecd, personnel_new.poststat, COUNT(personnel_new.personcd) FROM office INNER JOIN personnel_new ON office.officecd=personnel_new.officecd GROUP BY office.officecd, personnel_new.poststat ORDER BY office.officecd, personnel_new.poststat") or die($mysqli->error);
	...
	<tr>
        ...
        ?>
            ...
            <td class="text-center"><a href="pp_data_post_random/first_appt_letter_office_new.php?office_code=<?php echo $blockmuni_office[$i]['officecd']; ?>" class="btn btn-default btn-sm text-red" target="_blank"><i class="fa fa-print"></i></a></td> 
        </tr>
        <?php
        }
        ?>
