...
if($posted_date != 'ALL'){
    $first_app_query=$mysqli->prepare("SELECT personcd, officer_name, off_desg, poststatus, mob_no, epic, partno, slno, acno, bank, branch, ifsc, bank_accno, officecd, office, address, block_muni_name, postoffice, subdivision, policestation, district, pin, training_desc FROM first_rand_table_new WHERE subdivisioncd = ? AND DATE(posted_date) = ? ORDER BY officecd, personcd") or die($mysqli->error);
    $first_app_query->bind_param("ss",$subdiv_code,$posted_date) or die($first_app_query->error);
}
else{
    $first_app_query=$mysqli->prepare("SELECT personcd, officer_name, off_desg, poststatus, mob_no, epic, partno, slno, acno, bank, branch, ifsc, bank_accno, officecd, office, address, block_muni_name, postoffice, subdivision, policestation, district, pin, training_desc FROM first_rand_table_new WHERE subdivisioncd = ? ORDER BY officecd, personcd") or die($mysqli->error);
