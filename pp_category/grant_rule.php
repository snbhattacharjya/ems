...
$mysqli->query($grant_rule_query) or die(json_encode(array("Status"=>$mysqli->error)));
$rows_affected=$mysqli->affected_rows;

$mysqli->query("UPDATE pp_post_rules SET Recordsaffected=$rows_affected, AppliedDate='$today' WHERE RuleID=$rule_id") or die(json_encode(array("Status"=>$mysqli->error)));
$mysqli->close();

echo json_encode(array("Status"=>$rows_affected,"AppliedDate"=>$today));
?>