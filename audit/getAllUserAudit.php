...
$audit_query=$mysqli->prepare("SELECT application_audit.UserID, users.UserName, users.Designation, ObjectID, ObjectActivity, RequestIP, SessionID, ActivityTimeStamp FROM application_audit INNER JOIN users ON application_audit.UserID=users.UserID ORDER BY ActivityTimeStamp DESC") or die(json_encode(array("Status"=>"Error","Message"=>$mysqli->error)));
