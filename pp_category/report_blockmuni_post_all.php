...
$subdiv_query=$mysqli->prepare("SELECT block_muni.blockminicd, block_muni.blockmuni, COUNT(personnel.personcd) FROM (block_muni INNER JOIN office ON block_muni.blockminicd = office.blockormuni_cd) INNER JOIN personnel ON personnel.officecd=office.officecd WHERE block_muni.blockminicd != '999901' AND personnel.poststat !='' GROUP BY block_muni.blockminicd, block_muni.blockmuni ORDER BY block_muni.blockminicd, block_muni.blockmuni") or die($mysqli->error);
...
<table id="subdiv_booked_summary" class="table table-bordered table-condensed table-hover small">
    ...
    <tbody>
        <?php
        $subdiv_booked_query=$mysqli->prepare("SELECT block_muni.blockminicd, personnel.poststat, COUNT(personnel.personcd) FROM (block_muni INNER JOIN office ON block_muni.blockminicd = office.blockormuni_cd) INNER JOIN personnel ON personnel.officecd=office.officecd WHERE personnel.poststat != '' GROUP BY block_muni.blockminicd, personnel.poststat ORDER BY block_muni.blockminicd, personnel.poststat") or die($mysqli->error);
