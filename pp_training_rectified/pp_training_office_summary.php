...
$blockmuni_office_query=$mysqli->prepare("SELECT office.officecd, office.office, office.address1, COUNT(personnel_rectified.personcd) FROM office INNER JOIN personnel_rectified ON office.officecd=personnel_rectified.officecd WHERE office.blockormuni_cd = ? AND personnel_rectified.poststat IN ('PR','P1','P2','P3','PA') AND personnel_rectified.booked IN ('P','R') GROUP BY office.officecd, office.office, office.address1 ORDER BY office.officecd") or die($mysqli->error);
...
<div class="text-center">
    <a class="btn btn-default btn-md" href="pp_training_rectified/pp_training_office_summary_print.php?blockmuni=<?php echo $blockmuni_param; ?>" target="_blank">
...
<table id="blockmuni_office_summary" class="table table-bordered table-condensed table-hover small">
    ...
    <tbody>
        <?php
        $blockmuni_office_booked_query=$mysqli->prepare("SELECT office.officecd, personnel_rectified.poststat, COUNT(personnel_rectified.personcd) FROM office INNER JOIN personnel_rectified ON office.officecd=personnel_rectified.officecd WHERE office.blockormuni_cd = ? AND personnel_rectified.poststat IN ('PR','P1','P2','P3','PA') AND personnel_rectified.booked IN ('P','R') GROUP BY office.officecd, personnel_rectified.poststat ORDER BY office.officecd, personnel_rectified.poststat") or die($mysqli->error);
	...
	<tr>
        ...
        ?>
            ...
            <td class="text-center"><a href="pp_training_rectified/first_appointment_letter_office.php?office_code=<?php echo $blockmuni_office[$i]['officecd']; ?>" class="text-red" target="_blank"><i class="fa fa-print"></i> Print</a></td>
        </tr>
        <?php
        }
        ?>
...
<script>
    ...
    function loadBlockMuniBookedSummary(subdiv){
        ...
        $.ajax({
                ...
                url: "pp_training_rectified/pp_training_blockmuni_summary.php",
                type: "POST",
                data: {
                    subdiv: subdiv
                },
    ...
    function loadPPOfficeReport(officecd, blockmuni, subdiv){
        ...
        $.ajax({
                mimeType: 'text/html; charset=utf-8', // ! Need set mimeType only when run from local file
                url: "pp_training_rectified/pp_booked_by_office.php",
                type: "POST",
                data: {
                    officecd: officecd,
                    blockmuni: blockmuni,
