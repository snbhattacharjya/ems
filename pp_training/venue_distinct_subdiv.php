...
if(!isset($_SESSION['Subdiv'])){
    $venue_details_query=$mysqli->prepare("SELECT DISTINCT venue_base_name FROM training_venue ORDER BY venue_base_name") or die($mysqli->error);
}
else{
    ...
    $venue_details_query->bind_param("s",$_SESSION['Subdiv']) or die($venue_details_query->error);
}
