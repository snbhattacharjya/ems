...
$poststat_query=$mysqli->prepare("SELECT poststat.post_stat, poststat.poststatus, COUNT(personnel.personcd) FROM ((office INNER JOIN personnel ON office.officecd=personnel.officecd) INNER JOIN poststat ON poststat.post_stat=personnel.poststat) INNER JOIN assembly ON assembly.assemblycd = personnel.acno WHERE personnel.poststat IN ('PR','P1','P2','P3') AND personnel.booked IN ('P','R') AND office.blockormuni_cd = ? AND assembly.assemblycd != '999' GROUP BY poststat.post_stat, poststat.poststatus ORDER BY poststat.post_stat, poststat.poststatus") or die($mysqli->error);
...
<div class="text-center margin-bottom">
    <a class="btn btn-default btn-md" href="pp_training/pp_form12_office_summary_print.php?blockmuni=<?php echo $blockmuni_param; ?>" target="_blank">
...
<table id="blockmuni_office_summary" class="table table-bordered table-condensed table-hover small">
    ...
    <tbody>
        <?php
        $blockmuni_office_booked_query=$mysqli->prepare("SELECT office.officecd, personnel.poststat, COUNT(personnel.personcd) FROM (office INNER JOIN personnel ON office.officecd=personnel.officecd) INNER JOIN assembly ON assembly.assemblycd = personnel.acno WHERE office.blockormuni_cd = ? AND personnel.poststat IN ('PR','P1','P2','P3') AND personnel.booked IN ('P','R') AND assembly.assemblycd != '999' GROUP BY office.officecd, personnel.poststat ORDER BY office.officecd, personnel.poststat") or die($mysqli->error);
	...
	<tr>
        ...
        ?>
            ...
            <td class="text-center"><a href="pp_training/form12_print_office.php?office_code=<?php echo $blockmuni_office[$i]['officecd']; ?>" class="text-red" target="_blank"><i class="fa fa-print"></i> Print</a></td>
        </tr>
        <?php
        }
        ?>
