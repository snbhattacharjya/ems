...
//Checking Unique bank_acc_no in personnel
...
//Checking Unique bank_acc_no in personnel_new
$bank_acc_query=$mysqli->prepare("SELECT COUNT(*) FROM personnel_new WHERE bank_acc_no = ? AND personcd != ?") or die(json_encode(array("Status"=>$mysqli->error)));
$bank_acc_query->bind_param("ss",$bank_acc_no,$emp_code) or die(json_encode(array("Status"=>$mysqli->error)));
$bank_acc_query->execute() or die(json_encode(array("Status"=>$bank_acc_query->error)));
$bank_acc_query->bind_result($check_bank_acc) or die(json_encode(array("Status"=>$bank_acc_query->error)));
$bank_acc_query->fetch() or die(json_encode(array("Status"=>$bank_acc_query->error)));
...
if($status == 'NEW'){
    $update_bank_query=$mysqli->prepare("UPDATE personnel_new SET bank_cd = ?, branchname = ?, branchcd = ?, bank_acc_no = ? WHERE personcd = ?") or die(json_encode(array("Status"=>$mysqli->error)));
}
else if($status == 'OLD'){
