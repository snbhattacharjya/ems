...
$training_attendance_list_query=$mysqli->prepare("SELECT personnel_exempted.personcd, personnel_exempted.officer_name, personnel_exempted.off_desg, office.officecd, office.office, office.address1, office.address2, poststat.poststatus, personnel_exempted.mob_no, training_venue.venuename, training_schedule.training_dt, training_schedule.training_time FROM (((personnel_exempted INNER JOIN office ON personnel_exempted.officecd = office.officecd) INNER JOIN poststat ON personnel_exempted.poststat = poststat.post_stat) INNER JOIN training_schedule ON personnel_exempted.training1_sch = training_schedule.schedule_code) INNER JOIN training_venue ON training_venue.venue_cd = training_schedule.training_venue ORDER BY personnel_exempted.personcd, poststat.poststatus") or die($mysqli->error);
