...
if($opt == 'COUNT_PP'){
$counting_pp_query=$mysqli_countppds->prepare("SELECT assembly.assemblycd, assembly.assemblyname, pollingstation.psno, pollingstation.psname, pollingstation.groupid FROM  pollingstation INNER JOIN assembly ON assembly.assemblycd = pollingstation.forassembly WHERE pollingstation.forsubdivision = ? ORDER BY assembly.assemblycd, pollingstation.groupid") or die($mysqli_countppds->error);
}
