...
if($status == 'OLD'){
    $update_ppds_personnel_query=$mysqli->prepare("UPDATE ppds.personnel SET email = ?, resi_no = ?, mob_no = ? WHERE personcd = ?") or die(json_encode(array("Status"=>$mysqli->error)));
    $update_ppds_personnel_query->bind_param("ssss",$email,$phone,$mobile,$emp_code) or die(json_encode(array("Status"=>$update_ppds_personnel_query->error)));
    ...
    $update_ppds_personnel_query->close();
     ...
     $update_ppds_personnela_query=$mysqli->prepare("UPDATE ppds.personnela SET email = ?, resi_no = ?, mob_no = ? WHERE personcd = ?") or die(json_encode(array("Status"=>$mysqli->error)));
    $update_ppds_personnela_query->bind_param("ssss",$email,$phone,$mobile,$emp_code) or die(json_encode(array("Status"=>$update_ppds_personnela_query->error)));
