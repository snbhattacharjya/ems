...
<h3 class="page-header" style="display: none"><span class="search_val"><?php echo $search_val; ?></span><span class="search_param"><?php echo $search_param; ?></span></h3>
...
{
    var search_val = $('.search_val').html().toString();
    var search_param = $('.search_param').html().toString();
    $('.overlay').find('small').html(' Loading Employee Data...');
    $.ajax({
        mimeType: 'text/html; charset=utf-8', // ! Need set mimeType only when run from local file
        url: 'pp_counting/employee_by_office_import_search.php',
        type: "POST",
        data: {
        ...
        success: function(data) {
                ...
                if(emp.length > 0){
                    ...
                    $.each(emp, function(i){
                        $('#table_employee tbody').append("<tr><td><input type='checkbox' class='select-pp'></td><td class='personcd'>"+emp[i].personcd+"</td><td>"+emp[i].officer_name+"</td><td>"+emp[i].off_desg+"</td><td>"+emp[i].gender+"</td><td>"+emp[i].mob_no+"</td><td>"+emp[i].office_details+"</td><td>"+poststat_combo+"</td></tr>");
                    });
