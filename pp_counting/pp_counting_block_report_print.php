...
$counitng_personnel_query->bind_result($personcd,$officer_name,$off_desg,$officecd,$office_name,$address1,$address2,$mobile,$scale,$basic_pay,$grade_pay,$group,$remarks) or die($counitng_personnel_query->error);
$return=array();
while($counitng_personnel_query->fetch())
{
	$return[]=array("PersonID"=>$personcd,"OfficerName"=>$officer_name,"Designation"=>$off_desg,"OfficeID"=>$officecd,"OfficeName"=>$office_name,"Address1"=>$address1,"Address2"=>$address2,"Mobile"=>$mobile, "ScaleOfPay"=>$scale, "BasicPay"=>$basic_pay, "GradePay"=>$grade_pay, "Group"=>$group, "Remarks"=>$remarks);
}
?>
<html>
    <title>
