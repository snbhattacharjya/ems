...
$police_personnel_query=$mysqli->prepare("SELECT policestation.policestationcd, policestation.policestation, COUNT(*) FROM policestation INNER JOIN policestation_booth_personnel ON policestation.policestationcd = policestation_booth_personnel.policestationcd GROUP BY policestation.policestationcd, policestation.policestation") or die($mysqli->error);
...
<table id="employee_exempt_details" class="table table-bordered table-condensed table-striped small">
    ...
    <tbody>
        ...
        for($i = 0; $i < count($police_personnel); $i++){
            ...
            echo "<td class='text-center'><a class='btn btn-default text-red' href='police_randomisation/police_booth_allocation_print.php?policestationcd=".$police_personnel[$i]['policestationcd']."' target='_blank'><i class='fa fa-print'></i></a></td>";
            echo "</tr>";
        }
        ?>
    </tbody>
    <tfoot>
        <tr class="danger">
            ...
            <th class="text-center">
                <?php 
                    echo "<a class='btn btn-default text-red' href='police_randomisation/police_booth_allocation_print.php?policestationcd=ALL' target='_blank'><i class='fa fa-print'></i></a>";
                ?>
            </th>
         </tr>
        <tr class="info">
