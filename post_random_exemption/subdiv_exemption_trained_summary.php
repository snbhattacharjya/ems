...
$subdiv_query=$mysqli->prepare("SELECT subdivision.subdivisioncd, subdivision.subdivision, COUNT(personnel_exempt_post_random.personcd) FROM (subdivision INNER JOIN personnel ON personnel.subdivisioncd=subdivision.subdivisioncd) INNER JOIN personnel_exempt_post_random ON personnel.personcd = personnel_exempt_post_random.personcd AND personnel_exempt_post_random.personcd NOT IN (SELECT personcd FROM personnel_training_absent) WHERE subdivision.subdivisioncd != '9999' AND personnel.poststat != '' GROUP BY subdivision.subdivisioncd, subdivision.subdivision ORDER BY subdivision.subdivisioncd") or die($mysqli->error);
...
$poststat_query=$mysqli->prepare("SELECT poststat.post_stat, poststat.poststatus, COUNT(personnel_exempt_post_random.personcd) FROM (poststat INNER JOIN personnel ON poststat.post_stat=personnel.poststat) INNER JOIN personnel_exempt_post_random ON personnel.personcd = personnel_exempt_post_random.personcd WHERE personnel.poststat != '' AND personnel_exempt_post_random.personcd NOT IN (SELECT personcd FROM personnel_training_absent)GROUP BY poststat.post_stat, poststat.poststatus ORDER BY poststat.post_stat, poststat.poststatus") or die($mysqli->error);
...
<div class="text-center margin-bottom">
    <a class="btn btn-md btn-flat btn-default text-red" href="post_random_exemption/subdiv_exemption_trained_summary_print.php" target="_blank"><i class="fa fa-print"></i> Print</a>
</div>
