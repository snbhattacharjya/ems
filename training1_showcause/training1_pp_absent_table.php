...
$training_absent_list_query=$mysqli->prepare("SELECT personnel.personcd, personnel.officer_name, personnel.off_desg, office.officecd, office.office, office.address1, office.address2, poststat.poststatus, personnel.mob_no, training_venue.venuename, DATE_FORMAT(DATE(training_schedule.training_dt),'%D-%b-%Y (%a)'), training_schedule.training_time FROM ((((personnel INNER JOIN office ON personnel.officecd = office.officecd) INNER JOIN poststat ON personnel.poststat = poststat.post_stat) INNER JOIN personnel_training ON personnel.personcd = personnel_training.personcd) INNER JOIN training_schedule ON personnel_training.schedule_code = training_schedule.schedule_code) INNER JOIN training_venue ON training_venue.venue_cd = training_schedule.training_venue AND training_venue.subdivisioncd = ? AND training_venue.venue_base_name = ? AND training_schedule.training_dt = ? AND training_schedule.training_time = ? AND personnel.personcd IN (SELECT personcd FROM personnel_training_absent WHERE personnel_training_absent.training_type = ?) ORDER BY training_venue.venuename, training_schedule.training_dt, training_schedule.training_time, personnel.officer_name") or die($mysqli->error);
