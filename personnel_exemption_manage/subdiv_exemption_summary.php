...
$subdiv_query=$mysqli->prepare("SELECT subdivision.subdivisioncd, subdivision.subdivision, COUNT(personnel_exempt_marked.personcd) FROM (subdivision INNER JOIN personnel ON personnel.subdivisioncd=subdivision.subdivisioncd) INNER JOIN personnel_exempt_marked ON personnel.personcd = personnel_exempt_marked.personcd WHERE subdivision.subdivisioncd != '9999' AND personnel.poststat != '' GROUP BY subdivision.subdivisioncd, subdivision.subdivision ORDER BY subdivision.subdivisioncd") or die($mysqli->error);
...
$poststat_query=$mysqli->prepare("SELECT poststat.post_stat, poststat.poststatus, COUNT(personnel_exempt_marked.personcd) FROM (poststat INNER JOIN personnel ON poststat.post_stat=personnel.poststat) INNER JOIN personnel_exempt_marked ON personnel.personcd = personnel_exempt_marked.personcd WHERE personnel.poststat != '' GROUP BY poststat.post_stat, poststat.poststatus ORDER BY poststat.post_stat, poststat.poststatus") or die($mysqli->error);
...
<table id="subdiv_exempt_summary" class="table table-bordered table-condensed table-hover small">
    ...
    <tbody>
        <?php
        $subdiv_exempt_query=$mysqli->prepare("SELECT subdivision.subdivisioncd, personnel.poststat, COUNT(personnel_exempt_marked.personcd) FROM (subdivision INNER JOIN personnel ON subdivision.subdivisioncd=personnel.subdivisioncd) INNER JOIN personnel_exempt_marked ON personnel.personcd=personnel_exempt_marked.personcd WHERE personnel.poststat != '' GROUP BY subdivision.subdivisioncd, personnel.poststat ORDER BY subdivision.subdivisioncd, personnel.poststat") or die($mysqli->error);
