...
$multi_menu_title=$_POST['MultiMenuTitle'];
$multi_menu_icon=$_POST['MultiMenuIcon'];
...
$add_multi_menu_link_query=$mysqli->prepare("INSERT INTO user_type_menu (PageName, UserTypeID, MenuType, MenuOrder, SubMenuOrder,MultiMenuTitle, MultiMenuIcon, ModifiedDate) VALUES(?,?,2,?,1,?,?,now())") or die(json_encode(array("Status"=>"Error","Message"=>$mysqli->error)));

$add_multi_menu_link_query->bind_param("siiss",$page_name,$user_type_id,$max_menu_order,$multi_menu_title,$multi_menu_icon) or die(json_encode(array("Status"=>"Error","Message"=>$add_multi_menu_link_query->error)));

$add_multi_menu_link_query->execute() or die(json_encode(array("Status"=>"Error","Message"=>$add_multi_menu_link_query->error)));

$add_multi_menu_link_query->close();
