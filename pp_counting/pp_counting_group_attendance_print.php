...
if($opt == 'COUNT_PP'){
$counting_pp_query=$mysqli_countppds->prepare("SELECT personnela.personcd, personnela.officer_name, personnela.off_desg, office.officecd, office.office, office.address1, office.address2, poststat.poststatus, personnela.mob_no, personnela.forassembly, assembly.assemblyname, personnela.groupid, grp_dcrc.member, poststatorder.membno FROM ((((personnela INNER JOIN office ON personnela.officecd = office.officecd) INNER JOIN poststat ON personnela.poststat = poststat.post_stat) INNER JOIN assembly ON assembly.assemblycd = personnela.forassembly) INNER JOIN grp_dcrc ON grp_dcrc.forassembly = personnela.forassembly AND grp_dcrc.groupid = personnela.groupid) INNER JOIN poststatorder ON poststatorder.memberparty = grp_dcrc.member AND poststatorder.poststat = personnela.poststat WHERE personnela.booked IN ('P') AND personnela.forsubdivision = ? ORDER BY personnela.forassembly, personnela.groupid, poststatorder.membno") or die($mysqli_countppds->error);
}
...
$counting_pp_query->bind_result($personcd,$officer_name,$off_desg,$officecd,$office_name,$address1,$address2,$poststatus,$mobile,$forassembly,$assemblyname,$groupid,$member,$membno) or die($counting_pp_query->error);

$return=array();

while($counting_pp_query->fetch()){
	$return[]=array("PersonID"=>$personcd,"OfficerName"=>$officer_name,"Designation"=>$off_desg,"OfficeID"=>$officecd,"OfficeName"=>$office_name,"Address1"=>$address1,"Address2"=>$address2,"PostStatus"=>$poststatus,"Mobile"=>$mobile, "AssemblyCode"=>$forassembly, "AssemblyName"=>$assemblyname, "GroupID"=>$groupid, "MemberParty"=>$member, "MemberNo"=>$membno);
}
