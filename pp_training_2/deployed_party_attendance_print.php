...
$deployed_party_query=$mysqli->prepare("SELECT personnel.personcd, personnel.officer_name, personnel.off_desg, office.officecd, office.office, office.address1, poststat.poststatus, personnel.mob_no, personnel.forassembly, assembly.assemblyname, personnel.groupid, second_training_venue.venuename, second_training_date_time.training_dt, second_training_date_time.training_time, second_training_subvenue.subvenue FROM ((((((personnel INNER JOIN office ON personnel.officecd = office.officecd) INNER JOIN poststat ON personnel.poststat = poststat.post_stat AND personnel.poststat != 'MO') INNER JOIN assembly ON assembly.assemblycd = personnel.forassembly) INNER JOIN second_training_schedule ON personnel.training2_sch = second_training_schedule.schedule_code) INNER JOIN second_training_subvenue ON second_training_schedule.tr_subvenue_cd = second_training_subvenue.subvenue_cd) INNER JOIN second_training_venue ON second_training_subvenue.venue_cd = second_training_venue.venue_cd) INNER JOIN second_training_date_time ON second_training_schedule.datetimecd = second_training_date_time.datetime_cd WHERE personnel.booked IN ('P') AND personnel.groupid != 0 AND personnel.forassembly = ? ORDER BY personnel.groupid, poststat.poststat_order") or die($mysqli->error);
...
?>
<title>Polling Party Attendance List</title>
<h3>
    Polling Party Attendance List
</h3>
