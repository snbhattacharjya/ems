...
$subdiv_query=$mysqli->prepare("SELECT subdivision.subdivisioncd, subdivision.subdivision, COUNT(personnel_exempt.personcd) FROM subdivision INNER JOIN personnel_exempt ON personnel_exempt.subdivisioncd=subdivision.subdivisioncd WHERE subdivision.subdivisioncd != '9999' AND personnel_exempt.poststat != '' GROUP BY subdivision.subdivisioncd, subdivision.subdivision ORDER BY subdivision.subdivisioncd") or die($mysqli->error);
...
<div class="text-center margin-bottom">
    <a class="btn btn-md btn-flat btn-default text-red" href="post_random_exemption/subdiv_prerandom_exemption_summary_print.php" target="_blank"><i class="fa fa-print"></i> Print</a>
</div>
<table id="subdiv_exempt_summary" class="table table-bordered table-condensed table-hover small">
    ...
    <tbody>
        <?php
        $subdiv_exempt_query=$mysqli->prepare("SELECT subdivision.subdivisioncd, personnel_exempt.poststat, COUNT(personnel_exempt.personcd) FROM subdivision INNER JOIN personnel_exempt ON subdivision.subdivisioncd=personnel_exempt.subdivisioncd WHERE personnel_exempt.poststat != '' GROUP BY subdivision.subdivisioncd, personnel_exempt.poststat ORDER BY subdivision.subdivisioncd, personnel_exempt.poststat") or die($mysqli->error);
