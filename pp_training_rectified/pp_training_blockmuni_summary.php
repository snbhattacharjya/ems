...
<div class="text-center margin-bottom">
    <a class="btn btn-default btn-md" href="pp_training_rectified/pp_training_blockmuni_summary_print.php?subdiv=<?php echo $subdiv_param; ?>" target="_blank">
...
<table id="blockmuni_exempt_summary" class="table table-bordered table-condensed table-hover small">
    ...
    <tbody>
        <?php
        $blockmuni_booked_query=$mysqli->prepare("SELECT block_muni.blockminicd, personnel_rectified.poststat, COUNT(personnel_rectified.personcd) FROM (office INNER JOIN personnel_rectified ON office.officecd=personnel_rectified.officecd) INNER JOIN block_muni ON office.blockormuni_cd=block_muni.blockminicd WHERE office.subdivisioncd = ? AND personnel_rectified.poststat IN ('PR','P1','P2','P3','PA') AND personnel_rectified.booked IN ('P','R') GROUP BY block_muni.blockminicd, personnel_rectified.poststat ORDER BY block_muni.blockminicd, personnel_rectified.poststat") or die($mysqli->error);
	...
	<tr>
        ...
        ?>
            ...
            <td class="text-center"><a href="pp_training_rectified/first_appointment_letter_blockmuni.php?block_muni_code=<?php echo $blockmuni[$i]['BlockMuniCode']; ?>" class="text-red" target="_blank"><i class="fa fa-print"></i> Print</a></td>
        </tr>
        <?php
        }
        ?>
...
<script>
    ...
    function loadSubdivPPBookedSummary(){
        ...
        $.ajax({
                ...
                url: "pp_training_rectified/pp_training_subdiv_summary.php",
                //type: "POST",
                //data: {
    ...
    function loadOfficePPBookedSummary(subdiv, blockmuni){
        ...
        $.ajax({
                mimeType: 'text/html; charset=utf-8', // ! Need set mimeType only when run from local file
                url: "pp_training_rectified/pp_training_office_summary.php",
                type: "POST",
                data: {
                    subdiv: subdiv,
                    blockmuni: blockmuni
